...
//	audio_module_data_write(&playback, tmp->len);
...
//    	enter_queue(queue[queue_num],data[i]);
//    	sem_post(&sem.audio_sem[queue_num]);

    	i++;
    	if(i==3)
    		i=0;

//    	connect_status[queue_num] = 1;
...
int main()
{
	 ...
	 * 队列初始化
	 */
	queue = (Plinkqueue)malloc(sizeof(Plinkqueue)*QUEUE_NUM);
