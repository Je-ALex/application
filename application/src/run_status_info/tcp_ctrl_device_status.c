...
int conf_status_reset_vote_status()
{
	...
	node_queue->con_status->sub_list[sub_num].v_result.assent = 0;
	node_queue->con_status->sub_list[sub_num].v_result.nay = 0;
	node_queue->con_status->sub_list[sub_num].v_result.waiver = 0;
	node_queue->con_status->sub_list[sub_num].v_result.timeout = 0;
...
int conf_status_calc_vote_result()
{
	...
	msg[0] = WIFI_MEETING_CON_VOTE;
	msg[1] = WIFI_MEETING_STRING;

	msg[2] = WIFI_MEETING_CON_V_ASSENT;
	msg[3] = (result[0]>>8) & 0xff;
	msg[4] = result[0] & 0xff;
	msg[5] = WIFI_MEETING_CON_V_NAY;
	msg[6] = (result[1]>>8) & 0xff;
	msg[7] = result[1] & 0xff;
	msg[8] = WIFI_MEETING_CON_V_WAIVER;
	msg[9] = (result[2]>>8) & 0xff;
	msg[10] = result[2] & 0xff;
	msg[11] = WIFI_MEETING_CON_V_TOUT;
	msg[12] = (result[3]>>8) & 0xff;
	msg[13] = result[3] & 0xff;

	conf_status_save_pc_conf_result(14,msg);
...
int conf_status_reset_score_status()
{
	...
	if(conf_status_get_pc_staus() > 0)
	{
		...
		if(sub_prop == WIFI_MEETING_CON_SUBJ_SCORE)
		{
			node_queue->con_status->sub_list[sub_num].scr_result.score = 0;
			node_queue->con_status->sub_list[sub_num].scr_result.num_peop = 0;
		}else{
			printf("%s-%s-%d the subject is not score subject\n",__FILE__,__func__,__LINE__);
			return ERROR;
		}
...
int conf_status_calc_score_result()
{
	...
	value = (double)node_queue->con_status->sub_list[sub_num].scr_result.score /
					(double)node_queue->con_status->sub_list[sub_num].scr_result.num_peop;
	node_queue->con_status->sub_list[sub_num].scr_result.score_r = value;

	sprintf(&msg[2],"%.1f",value);

	conf_status_save_pc_conf_result(sizeof(msg),(unsigned char*)msg);
