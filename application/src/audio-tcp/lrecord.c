...
void SNDWAV_Record(snd_data_format *sndpcm, snd_data_format *sndpcm_p,WAVContainer_t *wav, int fd,int port)
{
	off64_t rest;
	size_t c, frame_size;
...
//     snd_pcm_dump(playback.handle, playback.log);
	...
	SNDWAV_Record(&record, &playback,&wav, fd, 8080);
...
//	for(i=0;i<2;i++)
//	{
		ret = pthread_create(&th_a, NULL, audio_tcp_thread, NULL);
		if (ret != 0)
		{
