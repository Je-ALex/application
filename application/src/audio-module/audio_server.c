...
static void* audio_recv_thread(void* p)
{
    ...
    while(1){
		...
		if(node_queue->con_status->spk_number)
		{
			data[i]->msg = buffer[i];
	...
	//    	printf("%d:recv_num = %d\n",port,playback.recv_num);
			...
			enqueue(queue[queue_num],data[i]);
...
static void* audio_data_mix_thread(void* data)
{
    ...
    while(1)
    {
    	 ...
    	 * 发言的顺序是从低到高排序
    	 */
    	for(i=0;i<node_queue->con_status->spk_number;i++)
    	{
//    		sem_wait(&sem.audio_mix_sem[i]);

    		tmp[i] = dequeue(queue[i]);
		...
		if(j>0)
		{
			...
			printf("audio_tcp_mix [%d]recv_num=%d\n",j,length);
...
int wifi_sys_audio_init()
{
	 ...
	 * 队列初始化
	 */
	queue = (Paudio_queue)malloc(sizeof(Paudio_queue)*QUEUE_NUM);
